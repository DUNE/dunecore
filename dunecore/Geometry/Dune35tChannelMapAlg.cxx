...
cout << "XXXX: " << cs << " " << apa << " " << p << ": "
     ...
     << setw(4) << fFirstChannelInNextPlane[cs][apa][p] << endl;
      }// end plane loop
    }// end apa loop
  }// end cs
  ...
  unsigned int Dune35tChannelMapAlg::Nchannels(readout::ROPID const& ropid) const {
  ...
  //----------------------------------------------------------------------------
  double Dune35tChannelMapAlg::WireCoordinate
...
//    const double rotate = (planeid.TPC % 2 == 1)? -1.: +1.;
  ...
  //----------------------------------------------------------------------------
  WireID Dune35tChannelMapAlg::NearestWireID
  ...
  //----------------------------------------------------------------------------
  raw::ChannelID_t Dune35tChannelMapAlg::PlaneWireToChannel
  ...
  //----------------------------------------------------------------------------
  unsigned int Dune35tChannelMapAlg::NTPCsets
    (readout::CryostatID const& cryoid) const
  {
    return 4;
  } // Dune35tChannelMapAlg::NTPCsets()
  
  
  //----------------------------------------------------------------------------
  unsigned int Dune35tChannelMapAlg::MaxTPCsets() const
  {
    return 4;
  } // Dune35tChannelMapAlg::MaxTPCsets()
  
  
  //----------------------------------------------------------------------------
  bool Dune35tChannelMapAlg::HasTPCset(readout::TPCsetID const& tpcsetid) const
  {
    return tpcsetid.TPCset < NTPCsets(tpcsetid);
  } // Dune35tChannelMapAlg::HasTPCset()
  
  
  //----------------------------------------------------------------------------
  readout::TPCsetID Dune35tChannelMapAlg::TPCtoTPCset
    (geo::TPCID const& tpcid) const
  {
    throw cet::exception("Dune35tChannelMapAlg") << __func__ << " not implemented!\n";
  } // Dune35tChannelMapAlg::TPCtoTPCset()
  
  
  //----------------------------------------------------------------------------
  std::vector<geo::TPCID> Dune35tChannelMapAlg::TPCsetToTPCs
    (readout::TPCsetID const& tpcsetid) const
  {
    throw cet::exception("Dune35tChannelMapAlg") << __func__ << " not implemented!\n";
  } // Dune35tChannelMapAlg::TPCsetToTPCs()
  
  
  //----------------------------------------------------------------------------
  geo::TPCID Dune35tChannelMapAlg::FirstTPCinTPCset
    (readout::TPCsetID const& tpcsetid) const
  {
    throw cet::exception("Dune35tChannelMapAlg") << __func__ << " not implemented!\n";
  } // Dune35tChannelMapAlg::FirstTPCinTPCset()
  
  
  //----------------------------------------------------------------------------
  unsigned int Dune35tChannelMapAlg::NROPs
      (readout::TPCsetID const& tpcsetid) const
  {
    return 4;
    //throw cet::exception("Dune35tChannelMapAlg") << __func__ << " not implemented!\n";
  } // Dune35tChannelMapAlg::NROPs()
  
  
  //----------------------------------------------------------------------------
  unsigned int Dune35tChannelMapAlg::MaxROPs() const {
    return 4;
  } // Dune35tChannelMapAlg::MaxROPs()
  
  
  //----------------------------------------------------------------------------
  bool Dune35tChannelMapAlg::HasROP(readout::ROPID const& ropid) const {
    return ropid.ROP < NROPs(ropid);
  } // Dune35tChannelMapAlg::HasROP()
  
  
  //----------------------------------------------------------------------------
  readout::ROPID Dune35tChannelMapAlg::WirePlaneToROP
    (geo::PlaneID const& planeid) const
  {
    throw cet::exception("Dune35tChannelMapAlg") << __func__ << " not implemented!\n";
  } // Dune35tChannelMapAlg::WirePlaneToROP()
  
  
  //----------------------------------------------------------------------------

  std::vector<geo::PlaneID> Dune35tChannelMapAlg::ROPtoWirePlanes(readout::ROPID const& ropid) const {
    std::vector<geo::PlaneID> plaids;
    std::vector<geo::TPCID> tpcids = ROPtoTPCs(ropid);
    unsigned int irop = ropid.ROP;
    for ( TPCID tpcid : tpcids ) {
      if ( irop == 0 || irop == 1 ) plaids.push_back(geo::PlaneID(tpcid, irop));
      else if ( irop == 2 || irop == 3 ) plaids.push_back(geo::PlaneID(tpcid, 2));
    }
    return plaids;
  }
  ...
  std::vector<geo::TPCID> Dune35tChannelMapAlg::ROPtoTPCs(readout::ROPID const& ropid) const {
  ...
  //----------------------------------------------------------------------------
  readout::ROPID Dune35tChannelMapAlg::ChannelToROP
    (raw::ChannelID_t channel) const
  {
    throw cet::exception("Dune35tChannelMapAlg") << __func__ << " not implemented!\n";
  } // Dune35tChannelMapAlg::ROPtoTPCs()
  
  
  //----------------------------------------------------------------------------

  raw::ChannelID_t Dune35tChannelMapAlg::FirstChannelInROP(readout::ROPID const& ropid) const {
    unsigned int icry = ropid.Cryostat;
    unsigned int iapa = ropid.TPCset;
    unsigned int irop = ropid.ROP;
  ...
  geo::PlaneID Dune35tChannelMapAlg::FirstWirePlaneInROP(readout::ROPID const& ropid) const {
    geo::PlaneID pid = ROPtoWirePlanes(ropid).at(0);
    return pid;
  }
