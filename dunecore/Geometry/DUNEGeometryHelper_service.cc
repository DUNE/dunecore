...
doConfigureChannelMapAlg(fhicl::ParameterSet const& pset, geo::GeometryCore* geom) {
  ...
  if ( fChannelMapClass.size() ) {
    ...
    } else if ( fChannelMapClass == "ProtoDUNEChannelMapAlgv6" ) {
      fChannelMap = std::make_shared<geo::ProtoDUNEChannelMapAlgv6>(pset);
    } else {
  ...
  // Otherwise derive class name from detector name.
    ...
    } else if ( detectorName.find("protodunev6") != std::string::npos ) {
      fChannelMap = std::make_shared<geo::ProtoDUNEChannelMapAlgv6>(pset);
