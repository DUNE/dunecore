...
# and open it
    $DEF = "iceberg_v2_Def" . $suffix . ".gdml";
    push (@gdmlFiles, $DEF);
    $DEF = ">" . $DEF;
...
# and open it
    $MAT = "iceberg_v2_Materials" . $suffix . ".gdml";
    push (@gdmlFiles, $MAT);
    $MAT = ">" . $MAT;
...
# add file to list of output GDML fragments,
# and open it
    $TPC = "iceberg_v2_TPC_${_[3]}" . $suffix . ".gdml";
    push (@gdmlFiles, $TPC);
    $TPC = ">" . $TPC;
...
# Number of wires per top that aren't cut off by the corner -- different from the
# DUNE APA in that these go from top to bottom, not side to side
# historically, "Corner" meant number of wires anchored on the top edge on one side.
# and Common wires means the number of U or V wires that go straight from the top
# edge to the bottom without touching the side

$NumberCommonUWires = $NumberCornerUWires - $NumberSideUWires;
$NumberCommonVWires = $NumberCornerVWires - $NumberSideVWires;
...
# hard coded number will be a factor determined from engineering spreadsheets on wire endpoints,
...
my $FirstTopUWire_yspan_iceberg = $Uactive_y;
...
my $FirstTopVWire_yspan_iceberg = $Uactive_y;
    
    # The corner wires for the U plane -- actually limited by the wires hitting the side
    
if ($wires_on==1)
{
    for ($i = 0; $i < $NumberSideUWires; $i++)
    {
...
EOF
    ...
    $CommonUWireLength = $Uactive_y/$CosUAngle;
...
EOF
	...
	$TopCornerUWireLength[$i] = ($FirstTopUWire_yspan_iceberg - $i*$UWire_yint - $UWire_yint)/$CosUAngle;
...
# The corner wires for the V plane -- really side wires
if ($wires_on==1) 
{
    for ($i = 0; $i < $NumberSideVWires; ++$i)
    {
...
EOF
    ...
    $CommonVWireLength = ($Vactive_y - $VWire_yint )/$CosVAngle;
...
EOF
	...
	$TopCornerVWireLength[$i] = ($FirstTopVWire_yspan_iceberg - $i*$VWire_yint - 2.5*$VWire_yint)/$CosVAngle;
        if ($TopCornerVWireLength[$i] < 0)
	{
	    $TopCornerVWireLength[$i] = 0.5;
	}
...
EOF
  ...
  # Corner U wires logical volumes -- really count up to the side wires
  for ($i = 0; $i < $NumberSideUWires; ++$i)
  {
  print TPC <<EOF;
...
EOF
  ...
  for ($j = $NumberCornerUWires; $j < $NumberSideUWires + $NumberCornerUWires; ++$j)
  {
  print TPC <<EOF;
...
EOF
  ...
  for ($j = $NumberCornerVWires; $j < $NumberSideVWires + $NumberCornerVWires; ++$j)
  {
  print TPC <<EOF;
...
# Starting with the bottom left corner wires:
  ...
  #  There are only NumberSideUWires in the corner.  "Corner" really means top and bottom edge here
...
# Moving sideways to the common wires:
...
my $StartCommonUWires_zpos = $lastZpos - $UWire_zint;

    print "TRJ corner, side U: $NumberCornerUWires   $NumberSideUWires\n";
    
for ($i = $NumberSideUWires; $i < $NumberCornerUWires; ++$i)
{
    $j = $i - $NumberSideUWires;
    my $zpos = $StartCommonUWires_zpos - $UWire_zint*($j);
    my $ypos = $lastYpos;

    $lastWnum = $i-1;
#    if ( $ypos <  $lastYpos ){  print "WARNING2: y position dropped from $lastYpos (wire U$lastWnum) to $ypos (wire U$i)\n"; }
#    if ( $ypos == $lastYpos ){  print "WARNING2: y position between wire U$lastWnum and U$lastWnum did not move: $ypos\n"; }
...
my $FirstTopUWire_zspan = $FirstTopUWire_yspan*$TanUAngle;
my $StartTopUWires_ypos =  $lastYpos + $UWire_yint/2.0;
my $StartTopUWires_zpos =  $lastZpos - $UWire_zint/2.0;
...
$i = $j - $NumberCornerUWires;

my $ypos = $StartTopUWires_ypos + ($i)*0.5*$UWire_yint;
my $zpos = $StartTopUWires_zpos - ($i)*0.5*$UWire_zint;

    $lastWnum = $j-1;
    if ( $ypos <  $lastYpos ){  print "WARNING3: y position dropped from $lastYpos (wire U$lastWnum) to $ypos (wire U$j)\n"; }
    if ( $ypos == $lastYpos ){  print "WARNING3: y position between wire U$lastWnum and U$j did not move: $ypos\n"; }
...
# Starting with the bottom right corner wires. "Corner" really means top and bottom edge here
   # x=0 to center the wires in the plane
   # y positioning: (-0.5*$TPCWirePlaneHeight) starts the incremental increase
        # from the bottom of the plane, and trigonometry gives the increment
   # z positioning: Looking at the plane from the positive x direction,
...
my $StartCommonVWires_zpos = $lastZpos + $VWire_zint;

    print "TRJ corner, side V: $NumberCornerVWires   $NumberSideVwires\n";
    
for ($i = $NumberSideVWires; $i < $NumberCornerVWires; ++$i)
{

    $j = $i - $NumberSideVWires;
    my $zpos = $StartCommonVWires_zpos + $VWire_zint*($j);
    my $ypos = $lastYpos;
...
my $FirstTopVWire_zspan = $FirstTopVWire_yspan*$TanVAngle;
my $StartTopVWires_ypos =  $lastYpos + $VWire_yint/2.0;
my $StartTopVWires_zpos =  $lastZpos + $VWire_zint/2.0;
...
# add file to list of output GDML fragments,
# and open it
    $CRYO = "iceberg_v2_Cryostat" . $suffix . ".gdml";
    push (@gdmlFiles, $CRYO);
    $CRYO = ">" . $CRYO;
...
# add file to list of output GDML fragments,
# and open it
    $ENCL = "iceberg_v2_DetEnclosure" . $suffix . ".gdml";
    push (@gdmlFiles, $ENCL);
    $ENCL = ">" . $ENCL;
...
# add file to list of output GDML fragments,
# and open it
    $WORLD = "iceberg_v2_World" . $suffix . ".gdml";
