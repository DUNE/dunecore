...
#pmts not equally spaced:
  ...
  #Beam plug is in z<0 and y>0
  $HorizontalPMTpos_bot = -301.7;
  $HorizontalPMTpos_top = -225.9;
  $pos11 = " x=\"$HorizontalPMTpos_top\" z=\"256.9\" y=\"221.0\" "; # Horizontal PMTs nonTCO side PEN
  $pos12 = " x=\"$HorizontalPMTpos_bot\" z=\"256.9\" y=\"221.0\" "; # Horizontal PMTs nonTCO side PEN

  $pos13 = " x=\"$HorizontalPMTpos_top\" z=\"256.9\" y=\"-221.0\" "; # Horizontal PMTs TCO side TPB
  $pos14 = " x=\"$HorizontalPMTpos_bot\" z=\"256.9\" y=\"-221.0\" "; # Horizontal PMTs TCO side TPB
  
  $pos21 = " x=\"$HorizontalPMTpos_top\" z=\"-256.9\" y=\"221.0+35.0\" "; # Horizontal PMTs near the beam plug TPB + offset
  $pos22 = " x=\"$HorizontalPMTpos_bot\" z=\"-256.9\" y=\"221.0+35.0\" "; # Horizontal PMTs near the beam plug TPB + offset

  $pos23 = " x=\"$HorizontalPMTpos_top\" z=\"-256.9\" y=\"-221.0\" "; # Horizontal PMTs TCO side TPB
  $pos24 = " x=\"$HorizontalPMTpos_bot\" z=\"-256.9\" y=\"-221.0\" "; # Horizontal PMTs TCO side TPB
...
EOF
        ...
        <physvol>
         ...
         <position name="posAnodePlateTop\-$idx" unit="cm" x="$posAnodePlateTop" y="@{[$CathodePosY]}" z="@{[$CathodePosZ]}"/>
         <rotationref ref="rIdentity"/>
        </physvol>
        <physvol>
         <volumeref ref="volAnodePlate"/>
         <position name="posAnodePlateBot\-$idx" unit="cm" x="$posAnodePlateBot" y="@{[$CathodePosY]}" z="@{[$CathodePosZ]}"/>
         <rotationref ref="rIdentity"/>
        </physvol>
...
for($i=0;$i<$nCRM_x/2;$i++){
  for($j=0;$j<$nCRM_z/2;$j++){
    ...
    if($ArapucaMesh_switch eq "on"){
      ...
      place_ResistiveMeshCathode($FrameCenter_x, $FrameCenter_y, $FrameCenter_z, $i, $j);
    }
    ...
    for ($mesh_y = 0; $mesh_y<4; $mesh_y++)
    {
     for ($mesh_z = 0; $mesh_z<4; $mesh_z++)
     {
  	...
  	if($mesh_z<4){
        ...
        <physvol>
        <volumeref ref="volCathodeMeshunion"/>
        <position name="posTopResistiveMesh$mesh_y\-$mesh_z-Frame\-$Frame_x\-$Frame_z" unit="cm"
        x="@{[$TopMesh_X]}"
        y="@{[$Mesh_Y]}"
        z="@{[$Mesh_Z]}"/>
        </physvol>
        <physvol>
        <volumeref ref="volCathodeMeshunion"/>
        <position name="posBotResistiveMesh$mesh_y\-$mesh_z-Frame\-$Frame_x\-$Frame_z" unit="cm"
        x="@{[$BotMesh_X]}"
        y="@{[$Mesh_Y]}"
        z="@{[$Mesh_Z]}"/>
        </physvol>
