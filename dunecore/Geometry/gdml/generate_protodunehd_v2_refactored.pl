...
$ArapucaDoubleAcceptanceWindow_x = 2.1; #double sided
...
#APA 3 single sided X-Arapucas (slots 2, 4, 8, 9 from top)
$nsingle1=8;
$nsingle2=6;
$nsingle3=2;
$nsingle4=1;

#APA 4 double sided X-Arapucas (slots 2, 4, 7, 9 from top)
$ndouble1=8;
$ndouble2=6;
$ndouble3=3;
$ndouble4=1;
...
# and open it
    $MAT = "pd2_v2_Materials" . $suffix . ".gdml";
    push (@gdmlFiles, $MAT);
    $MAT = ">" . $MAT;
...
# add file to list of output GDML fragments,
# and open it
    $TPC = "pd2_v2_TPC_${_[3]}" . $suffix . ".gdml";
    push (@gdmlFiles, $TPC);
    $TPC = ">" . $TPC;
...
# add file to list of output GDML fragments,
# and open it
    $CRYO = "pd2_v2_Cryostat" . $suffix . ".gdml";
    push (@gdmlFiles, $CRYO);
    $CRYO = ">" . $CRYO;
...
print CRYO <<EOF;
    ...
    <box name="ArapucaDoubleIn" lunit="cm"
      ...
      y="@{[$ArapucaAcceptanceWindow_y]}"
      z="@{[$ArapucaAcceptanceWindow_z]}"/>
     <subtraction name="ArapucaDoubleWalls0">
      <first  ref="ArapucaOut"/>
      <second ref="ArapucaDoubleIn"/>
      <positionref ref="posArapucaDoubleSub0"/>
      </subtraction>
     <subtraction name="ArapucaDoubleWalls1">
      <first  ref="ArapucaDoubleWalls0"/>
      <second ref="ArapucaDoubleIn"/>
      <positionref ref="posArapucaDoubleSub1"/>
      </subtraction>
     <subtraction name="ArapucaDoubleWalls2">
      <first  ref="ArapucaDoubleWalls1"/>
      <second ref="ArapucaDoubleIn"/>
      <positionref ref="posArapucaDoubleSub2"/>
      </subtraction>
     <subtraction name="ArapucaDoubleWalls">
      <first  ref="ArapucaDoubleWalls2"/>
      <second ref="ArapucaDoubleIn"/>
      <positionref ref="posArapucaDoubleSub3"/>
      </subtraction>
    <box name="ArapucaDoubleAcceptanceWindow" lunit="cm"
      x="@{[$ArapucaDoubleAcceptanceWindow_x]}"
      y="@{[$ArapucaAcceptanceWindow_y]}"
      z="@{[$ArapucaAcceptanceWindow_z]}"/>
...
#APA 1 and 2 with single sided X-Arapucas
for($i=0 ; $i<3 ; $i=$i+2){
for($p=0 ; $p<10 ; $p++){
...
#APA 3 with most double and some single sided X-Arapucas
$i=1;
for($p=0 ; $p<10 ; $p++){
for($ncuts=0 ; $ncuts<4; $ncuts++){
if($p==$nsingle1||$p==$nsingle2||$p==$nsingle3||$p==$nsingle4){
print CRYO <<EOF;
   <volume name="volOpDetSensitive_$i\-$p\-$ncuts">
     <materialref ref="LAr"/>
     <solidref ref="ArapucaSingleAcceptanceWindow"/>
...
#APA 4 with most single and some double sided X-Arapucas
$i=3;
for($p=0 ; $p<10 ; $p++){
for($ncuts=0 ; $ncuts<4; $ncuts++){
if($p==$ndouble1||$p==$ndouble2||$p==$ndouble3||$p==$ndouble4){
print CRYO <<EOF;
   <volume name="volOpDetSensitive_$i\-$p\-$ncuts">
     <materialref ref="LAr"/>
     <solidref ref="ArapucaDoubleAcceptanceWindow"/>
...
# add file to list of output GDML fragments,
# and open it
    $ENCL = "pd2_v2_DetEnclosure" . $suffix . ".gdml";
    push (@gdmlFiles, $ENCL);
    $ENCL = ">" . $ENCL;
...
# add file to list of output GDML fragments,
# and open it
    $WORLD = "pd2_v2_World" . $suffix . ".gdml";
