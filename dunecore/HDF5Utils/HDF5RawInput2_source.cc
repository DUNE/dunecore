...
#include "dunecore/HDF5Utils/HDF5RawInput2.h"
#include "dunecore/DuneObj/DUNEHDF5FileInfo2.h"
#include "lardataobj/RawData/RDTimeStamp.h"

dune::HDF5RawInput2Detail::HDF5RawInput2Detail(
...
void dune::HDF5RawInput2Detail::readFile(
...
bool dune::HDF5RawInput2Detail::readNext(art::RunPrincipal const* const inR,
                                         ...
                                         art::EventPrincipal*& outE) 
{
  ...
  if (run_id != run_id2)
    {
      throw cet::exception("HDF5RawInput2_source.cc") << " Inconsistent run IDs in file attribute and trigger record header " << run_id << " " << run_id2;
    }
  ...
  if (nextEventRecordID.first != trhtn)
    {
      throw cet::exception("HDF5RawInput2_source.cc") << " Inconsistent trigger record numbers: " << nextEventRecordID.first << " " << trhtn;
    }
  ...
  if (fLogLevel > 0)
    {
      std::cout << "HDF5RawInput2_source: Trigger Time Stamp: " << trigTimeStamp << std::endl;
    }
    
  uint64_t getTrigTime = formatTrigTimeStampWithFrequency(trigTimeStamp, fClockFreqMHz);
  if (fLogLevel > 0)
    {
      std::cout << "HDF5RawInput2_source: getTrigTime: " << getTrigTime << std::endl;
    }

  art::Timestamp artTrigStamp (getTrigTime);
  if (fLogLevel > 0)
    {
      std::cout << "HDF5RawInput2_source: artTrigStamp: " <<   artTrigStamp.value()<< std::endl;
    }
  ...
  if ( fHandleSequenceOption == "increment" )
    {
      ...
      if (fLogLevel > 0)
	{
	  std::cout << "HDF5RawInput2_source: assigning event number via incrment: " << event << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
	}
    }
  else if ( fHandleSequenceOption == "shiftadd" )
    {
      ...
      if (fLogLevel > 0)
	{
	  std::cout << "HDF5RawInput2_source: assigning event number via shfitadd: " << event << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
	}
    }
  else if ( fHandleSequenceOption == "ignore" )
    {
      if (fLogLevel > 0)
	{
	  std::cout << "HDF5RawInput2_source: assigning event number ignoring seqID: " << event << " orig: " << nextEventRecordID.first << " " << nextEventRecordID.second;
	}
    }
  else
    {
      throw cet::exception("HDF5RawInput2_source.cc") << "Ununderstood HandleSequenceOption: " << fHandleSequenceOption << " use: ignore, increment or shiftadd";
    }
  fLastEvent = event;
  ...
  if (fLogLevel > 0)
    {
      std::cout << "HDF5RawInput2_source: Event Time Stamp :" << outE->time().value() << std::endl;
    }
...
namespace dune {
  using HDF5RawInput2Source = art::Source<HDF5RawInput2Detail>;
}


DEFINE_ART_INPUT_SOURCE(dune::HDF5RawInput2Source)
