////////////////////////////////////////////////////////////////////////
// Class:       FDHDDAQWIBEthPCAP
// Plugin Type: analyzer (Unknown Unknown)
// File:        FDHDDAQWIBEthPCAP_module.cc
//
...
class FDHDDAQWIBEthPCAP : public art::EDAnalyzer {
public:
  explicit FDHDDAQWIBEthPCAP(fhicl::ParameterSet const& p);

  // Plugins should not be copied or assigned.
  FDHDDAQWIBEthPCAP(FDHDDAQWIBEthPCAP const&) = delete;
  FDHDDAQWIBEthPCAP(FDHDDAQWIBEthPCAP&&) = delete;
  FDHDDAQWIBEthPCAP& operator=(FDHDDAQWIBEthPCAP const&) = delete;
  FDHDDAQWIBEthPCAP& operator=(FDHDDAQWIBEthPCAP&&) = delete;
...
FDHDDAQWIBEthPCAP::FDHDDAQWIBEthPCAP(fhicl::ParameterSet const& p)
  : EDAnalyzer{p}  // ,
...
void FDHDDAQWIBEthPCAP::analyze(art::Event const& e)
{
...
// construct output filename from fOutfilenameBase_run<nnnnnn>_sr<nnn>_event<nnnnnn>.dat
  ...
  for (uint32_t iptn=0; iptn<RawDigits.size(); ++iptn)
      ...
      if (nSamples == 0)
	{
      ...
      if (nSamples != nSc)
	{
	  throw cet::exception("FDHDDAQWIBEthPCAP") << "raw digits have different numbers of samples: " 
						     << nSamples << " " <<  nSc;
	}
    }
	  ...
	  for (int streamindex=0; streamindex<8; ++streamindex)
	      ...
	      for (int streamchan=0; streamchan<64; ++streamchan)
		{
		  ...
		  if (!cinfo.valid)  // shouldn't happen -- map should have everything we can loop over
		    {
		      throw cet::exception("FDHDDAQWIBEthPCAP") << "electronics map does not have " << det_id << " " << crate << " " << slot << " " << stream << " " << streamchan;
		    }
	      ...
	      for (size_t isample=0; isample<nSamples; ++isample)
		{
		  ...
		  for (int streamchan=0; streamchan<64; ++streamchan)
		      ...
		      if (d<0)
			{
			  ...
			  if (!warnedNegative)
			    {
			      MF_LOG_WARNING("FDHDDAQWIBEthPCAP_module") << "Negative ADC value in raw::RawDigit.  Setting to zero to put in WIB frame\n";
			      warnedNegative = true;
			    }
			}
...
DEFINE_ART_MODULE(FDHDDAQWIBEthPCAP)
